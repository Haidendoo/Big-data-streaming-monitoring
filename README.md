...
4. Truy cập Metabase tại [http://localhost:3001](http://localhost:3001), tiến hành thiết lập kết nối:
   * **Database type**: Chọn **Presto**.
   * **Host**: `trino.lakehouse.svc.cluster.local` (hoặc `localhost` nếu dùng port-forward trực tiếp).
   * **Port**: `8888`
   * **Database name**: `iceberg`
   * **Schema**: `monitoring`
   * **Username**: `admin`
   * **Password**: *Để trống hoàn toàn*.
   * **SSL**: **Tắt (Disabled)** (Bắt buộc tắt SSL để tránh lỗi Protocol handshake do Trino chạy HTTP thuần).
5. Dùng bảng `kpi_summary` để dựng các biểu đồ giám sát và dashboard BI theo `province`, `station`, `window_start`.

> [!TIP]
> **Tối ưu hóa hiệu năng Spark Batch**: Hệ thống đã được tích hợp bộ nhớ đệm `spark-ivy-pvc`. Lần chạy đầu tiên Spark sẽ tải các gói JAR (Iceberg, AWS SDK) và lưu trữ lâu dài. Từ lần chạy thứ hai trở đi, thời gian thực thi DAG sẽ giảm từ **20 phút** xuống chỉ còn **15 giây** do dữ liệu thư viện đã được lấy trực tiếp từ cache PVC.
...
### 1. Hướng dẫn xử lý sự cố chung & Tối ưu hóa hệ thống
...
> [!IMPORTANT]
> **Khắc phục vĩnh viễn lỗi `too many open files` (fsnotify watcher):**
> Khi chạy cùng lúc nhiều dịch vụ (Flink, NiFi, Spark), hệ thống Linux/WSL2 sẽ hết bộ đếm file. Để sửa triệt để, chạy lệnh sau trên máy host:
> ```bash
...
> [!TIP]
> **Mẹo truy cập ổn định trên Windows WSL2:**
> Nếu truy cập qua `localhost` gặp lỗi kết nối (do bug loopback binding của Windows WSL2 khi đổi mạng), hãy lấy IP của WSL2 bằng lệnh `ip addr show dev eth0` hoặc `hostname -I` và truy cập trực tiếp bằng IP đó (ví dụ: `http://<WSL2_IP>:3001` cho Metabase).

### 2. Sự cố nén file Compaction nhưng MinIO vẫn giữ file Parquet cũ
* **Triệu chứng**: Chạy Spark Compaction thành công nhưng MinIO vẫn giữ nguyên các file Parquet nhỏ cũ cùng với file Parquet lớn mới.
* **Nguyên nhân**: Iceberg giữ lại các file cũ để phục vụ tính năng du hành thời gian (Time Travel).
* **Khắc phục**: Bản script `iceberg_compaction.py` trong dự án đã được tích hợp sẵn thủ tục tự động `CALL iceberg.system.expire_snapshots(..., retain_last => 1)` ở cuối luồng để dọn sạch hoàn toàn các file rác vật lý khỏi MinIO ngay sau khi nén. Bạn không cần dọn dẹp thủ công.

### 3. Sự cố lệch Metadata bảng Iceberg (NoSuchKey / NotFoundException)
