...
```
              ...
              lakehouse/raw-file/                       topic: file-arrival-events
                    │                                         │
                    └────────────────────┬────────────────────┘
                                         ▼
...
2. NiFi sẽ phát hiện file mới từ SFTP, chuyển tiếp file vào **MinIO S3** tại bucket `lakehouse` (trong folder `raw-file`).
3. Đồng thời NiFi đẩy một JSON event thông báo sang Kafka topic `file-arrival-events` với nội dung dạng:
   ```json
   {
     "file_path": "s3a://lakehouse/raw-file/2026/06/23/08/metrics_20260623_150555.csv",
...
4. **Kiểm tra Logs:** Xem tiến trình parse file của Flink:
   ...
   *Kết quả mong đợi:*
   ```text
   Processing event: FileArrivalEvent{filePath='s3a://lakehouse/raw-file/.../metrics_xxx.csv', ...}
